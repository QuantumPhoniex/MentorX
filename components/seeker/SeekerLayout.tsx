...
const SeekerLayout: React.FC<SeekerLayoutProps> = ({ 
  children, 
  activeView, 
  onNavigate, 
  onLogout 
  ...
  const navItems = [
    ...
    { id: 'create-project', label: 'Create Project', icon: <PlusCircle size={20} /> },
    { id: 'book-session', label: 'Book a Session', icon: <Calendar size={20} /> },
    { id: 'ongoing-sessions', label: 'Ongoing Sessions', icon: <Clock size={20} /> },
    { id: 'past-sessions', label: 'Past Sessions', icon: <Archive size={20} /> },
  ];
      ...
      <aside className="w-full md:w-64 bg-white border-r border-slate-200 flex flex-col fixed md:relative h-16 md:h-screen z-40 bottom-0 md:bottom-auto">
        ...
        <div className="p-3 border-t border-slate-100 hidden md:block">
            <button
              ...
              className={`w-full flex items-center px-3 py-2.5 rounded-lg text-sm font-medium transition-colors mb-1 ${
                activeView === 'profile' 
                  ? 'bg-indigo-50 text-indigo-700' 
                  : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
              }`}
            >
              <span className={`mr-3 ${activeView === 'profile' ? 'text-indigo-600' : 'text-slate-400'}`}>
                <Settings size={20} />
              </span>
              Profile & Settings
            </button>
